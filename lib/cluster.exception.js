  ...
  function exception (error, instance) {
    ...
    if (history && history.keys.length) {
      ...
      history.keys.forEach(function addGraphs(key){
        ...
        for(graph in graphs) {
          details.graph[graph] = graphs[graph];
        }
      });
    }
    ...
    if (master._killed)
      details.cluster.killed = master_killed;

    if (master.children && master.children.length)
      details.cluster.workers = master.children.length;

    if (process.env.CLUSTER_WORKER)
      details.cluster.worker_id = process.env.CLUSTER_WORKER;
