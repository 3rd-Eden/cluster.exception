...
History.prototype.toGraph = function(key){
  var base = 'https://chart.googleapis.com/chart?cht=ls&chs=400x150&chd='
    , data = this.data[key]
    , i = data.length
    , points = []
    , result = {};
  ...
  switch (key){
    case 'memory':
      ['rss', 'vsize', 'heapUsed', 'heapTotal'].forEach(function(type){
        ...
        result[type] = base + 't:' + points.join(',') + '&chtt=memoryUsage+'+ type +'&chts=000000,14';
      });
      break;
      
    case 'freemem': case 'totalmem':
      while(i--) points.push(History.bytesToMb(data[i]));
      result[key] = base + 't:' + points.join(',') + '&chtt=Server+'+ key +'&chts=000000,14';
      break;
